                        ...
                        <motion.div
                            ...
                            className={`relative rounded-[2.5rem] p-10 flex flex-col transition-all duration-500 h-full border ${
                                isIndigo 
                                    ...
                                    : isCurrent 
                                        ? 'bg-white border-amber-500 shadow-xl ring-1 ring-amber-500' 
                                        : 'bg-white border-slate-100 hover:border-amber-200 hover:shadow-xl'
                            }`}
                        >
                            {isCurrent && (
                                <div className="absolute -top-4 left-1/2 -translate-x-1/2 bg-amber-500 text-slate-950 text-[10px] font-bold px-8 py-2 rounded-full tracking-[0.4em] shadow-xl z-10">
                                    CURRENT
                                </div>
                            )}

                            <div className="mb-10">
                                <span className={`text-[10px] font-bold tracking-[0.3em] uppercase mb-6 block italic ${isIndigo ? 'text-amber-500' : 'text-slate-400'}`}>
                                    {plan.badge}
                                </span>
                                ...
                                <p className={`text-[10px] font-bold tracking-[0.3em] uppercase mt-4 italic ${isIndigo ? 'text-indigo-400' : 'text-amber-600'}`}>
                                    {plan.description}
                                </p>
                            </div>
                            ...
                            <ul className="space-y-5 mb-12 flex-grow">
                                {plan.features.map((feature) => (
                                    <li key={feature} className="flex gap-4 text-sm font-medium opacity-80">
                                        <div className={`w-1.5 h-1.5 rounded-full mt-2 flex-shrink-0 ${isIndigo || plan.id === 'founding' ? 'bg-amber-500' : 'bg-slate-200'}`}></div>
                                        {feature}
                                    </li>
                                ))}
                            </ul>
                            ...
                            {plan.productId ? (
                                <CheckoutButton
                                    ...
                                    className={`w-full py-6 text-[10px] font-bold uppercase tracking-[0.4em] transition-all rounded-2xl shadow-xl text-center ${
                                        isIndigo 
                                            ? 'bg-amber-500 text-slate-950 hover:bg-white' 
                                            : 'bg-indigo-950 text-white hover:bg-amber-500 hover:text-slate-950'
                                    }`}
                                >
                                    {plan.buttonText}
                                </CheckoutButton>
