...
export default function VoiceLounge({
  ...
  const transcribeAndSend = async (blob: Blob, mimeType: string) => {
    ...
    try {
      ...
      if (!text) {
        setMicHint('Didn’t catch that — tap and try again.');
        return;
      }
      setMicHint(null);
      ...
      setMicHint('Couldn’t hear that — tap to try again.');
    } finally {
  ...
  const stopListening = useCallback((opts?: { discard?: boolean; force?: boolean }) => {
    ...
    if (!opts?.discard && !opts?.force && armedFor < LISTEN_ARM_MS) {
      setMicHint('Keep talking — tap again when you’re done.');
      return;
    }
  ...
  const startListening = async () => {
    ...
    // Already recording → stop & send (after arm window)
    ...
    try {
      ...
      recorder.onstop = () => {
        ...
        stopMediaTracks();
        if (discard) return;
        if (blob.size < 200 || recordedMs < MIN_SEND_MS) {
          setMicHint('Tap, speak, then tap again when you’re done.');
          return;
        }
      ...
      recordMaxTimerRef.current = setTimeout(() => {
        if (mediaRecorderRef.current?.state === 'recording') {
          // Max length hit — send what we have
  ...
  const statusLine = (() => {
    ...
    if (isListening) return 'Listening… tap when done';
    ...
    if (messages.length === 0) return `Hello, ${firstName}`;
    return 'Tap to talk';
  })();
